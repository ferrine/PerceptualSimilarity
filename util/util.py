...
def resize_image(img, max_size=256):
    ...
    # resize
    max_dim = max([Y, X])
    zoom_factor = 1.0 * max_size / max_dim
    img = zoom(img, [zoom_factor, zoom_factor, 1])

    return img


def resize_image_zoom(img, zoom_factor=1.0, order=3):
    if zoom_factor == 1:
        return img
    else:
...
def montage(
    ...
    mont_imgs = np.zeros(
        (
            np.uint((Y + PADY) * MM - PADY + EXTRA_PADY),
            np.uint((X + PADX) * NN - PADX + EXTRA_PADX),
            M,
        ),
        dtype=imgs.dtype,
    )
    mont_imgs = mont_imgs + backClr.flatten()[np.newaxis, np.newaxis, :].astype(
        mont_imgs.dtype
    )
    ...
    for ii in np.random.permutation(N):
        ...
        mont_imgs[
            np.uint(grid_mm[ii] * (Y + PADY)) : np.uint((grid_mm[ii] * (Y + PADY) + Y)),
            np.uint(grid_nn[ii] * (X + PADX)) : np.uint((grid_nn[ii] * (X + PADX) + X)),
            :,
        ] = imgs[:, :, :, ii]
